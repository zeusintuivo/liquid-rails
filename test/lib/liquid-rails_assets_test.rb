...
class Liquid::Rails::AssetsTest < ActiveSupport::TestCase
  def with_app(use_sprockets, code)
    Dir.mktmpdir do |dir|
      manifest_path = File.join(dir, "app", "assets", "config", "manifest.js")
      FileUtils.mkdir_p(File.dirname(manifest_path))
      File.write(manifest_path, "//= link_tree ../html .liquid")
      ...
      asset_path = File.join(dir, "app", "assets", "html", "test.liquid")
      FileUtils.mkdir_p(File.dirname(asset_path))
      File.write(asset_path, ".test\n  | hi")

      asset_path = File.join(dir, "app", "assets", "html", "test_helpers.liquid")
  ...
  if Sprockets::VERSION.to_i < 4
    test "should return HTML version when passing '.liquid' extension" do
      assert_equal '<div class="test">hi</div>', with_app(true, 'print DummyApp.assets["test.liquid"]')
    end
  end
  ...
  if Sprockets::VERSION.to_i >= 4
    test "should return Liquid version when passing '.liquid' extension" do
      assert_equal ".test\n  | hi", with_app(true, 'print DummyApp.assets["test.liquid"]')
    end
  end
